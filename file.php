...
$commentFeliPath = COMMENT_DIR . '/' . $imageFileName . '.txt';
...
if (!empty($_POST['comment'])) {
	...
	if (empty($errors)) {
      ...
      $vowels = array("\r\n", "\r", "\n", "\\r", "\\n", "\\r\\n");
      $comment = str_replace($vowels, "<br/>", $comment);
      $comment = date('d.m.y H:i') . ': ' . $comment;

		file_put_contents($commentFeliPath, $comment . "\n", FILE_APPEND);

		$messages[] = 'Комментарий добавлен';
	}
}

$comments = file_exists($commentFeliPath) 
	? file($commentFeliPath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES)
	: [];
 ?>
 <!doctype html>
<html lang="en">
...
<div class="container pt-4">
    ...
    <h1 class="col-12 col-sm-8 offset-sm-2"><a href="<?php echo URL; ?>">Галерея изображений</a></h1>
    ...
    <h2 class="col-12 col-sm-8 offset-sm-2">Файл <?php echo $imageFileName; ?></h2>
